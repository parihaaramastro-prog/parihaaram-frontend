...
export default function DetailedPredictionContent() {
    const params = useParams();
    const router = useRouter();
    const searchParams = useSearchParams();
    const lang = searchParams.get('lang') || 'en';

    // Ensure params are string (Next.js params can be string | string[])
    const type = Array.isArray(params?.type) ? params.type[0] : params?.type as string;
    const sign = Array.isArray(params?.sign) ? params.sign[0] : params?.sign as string;
